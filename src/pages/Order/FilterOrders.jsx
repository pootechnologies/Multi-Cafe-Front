...
const fetchOrderItems = async () => {
  const response = await axiosInstance.get(API_ENDPOINTS.ORDERITEMS);
      ...
      <div className="md:hidden mt-4 space-y-3">
        {displayOrderItems.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`bg-white rounded-lg border p-4 transition-all duration-200 ${expandedCards.size > 0 && !isExpanded ? "blur-sm" : ""
                }`}
            >
